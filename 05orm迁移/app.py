...
class User(db.Model):
    ...
    # password = db.Column(db.String(222),nullable = False)
...
class UserExtension(db.Model):
    ...
    #db.backref
    # 1. 在反向引用的时候，如果需要传递一些其他的参数，那么就需要用到这个函数，否则不需要使用，只要在relationship的backref参数上，
    # ，设置反向引用的名称就可以了。
    # 2. uselist=False, 代表反向引用的时候不是一个列表而是一个对象。
